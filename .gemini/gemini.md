...
- **Runner:** Use `vitest` for unit and integration tests.
- **Environment:** Use `jsdom` to simulate the browser environment for React components.
- **Libraries:**
    - `@testing-library/react`: For rendering components and simulating user events.
    - `@testing-library/jest-dom`: For DOM-specific matchers (e.g., `toBeInTheDocument`, `toHaveClass`).
- **File Structure:**
    - Co-locate test files with the source code: `MyComponent.tsx` should have a sibling `MyComponent.test.tsx`.
    - Use `.test.ts` for logic/utility testing and `.test.tsx` for component testing.
- **Configuration:**
    - Tests rely on `vite.config.ts` (or `vitest.config.ts`) for path resolution and plugins.
    - Ensure a setup file (e.g., `src/setupTests.ts`) imports `@testing-library/jest-dom` to extend Vitest matchers.
- **Writing Tests:**
    - **Philosophy:** Test functionality and user interaction, not implementation details.
    - **Queries:** Prefer accessible queries (`getByRole`, `getByLabelText`) over `getByTestId` or CSS selectors.
    - **Mocking:** Use `vi.fn()` for function mocks and `vi.mock()` for module mocking when isolating complex dependencies.
- **Execution:**
